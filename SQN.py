...
class Network:
    def __init__(self, dataset, config, retrain=False):
        ...
        if self.config.saving:
            if self.config.saving_path is None:
                self.saving_path = time.strftime('results/SQN_Conv_Log_%Y-%m-%d', time.gmtime())
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            ...
            print(interpolated_points)        # Concatenation
        ...
        # f_layer_fc1, rs_mapf1, rs_mapf2 = self.convtext(f_layer_fc1, inputs['neigh_idx'][0], inputs['xyz'][0],
        #                                                 batch_anno_xyz, is_training)
    ...
    def convtext(self, feature, neigh_idx, xyz, batch_anno_xyz, is_training):
        feature_sq = feature
        d_in = feature.get_shape()[-1].value
        weakly_w = weight_variable(
            [self.config.batch_size, self.config.num_kernels, feature.get_shape()[-1].value, d_in])
        with tf.device('/cpu:0'):

            weakly_w = tf.cond(is_training,
                               lambda: weakly_w,
                               lambda: weakly_w[0:self.config.val_batch_size, :, :, :], )
            weakly_w = tf.cond(is_training,
                               lambda: tf.concat([weakly_w, weakly_w], axis=0),
                               lambda: weakly_w)
            neigh_idx = tf.cond(is_training,
                               lambda: tf.concat([neigh_idx, neigh_idx], axis=0),
                               lambda: neigh_idx)
            xyz = tf.cond(is_training,
                               lambda: tf.concat([xyz, xyz], axis=0),
                               lambda: neigh_idx)
        neighbor_xyz = self.gather_neighbour(xyz, neigh_idx)
        neighbor_features = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)
        weakly_KP_extent = 1.0
        K_radius = 1.5 * weakly_KP_extent
        weakly_num_kpoints = self.config.num_kernels
        weakly_points_dim = tf.shape(neigh_idx)[-1]
        weakly_K_points_numpy = kernels.load_kernels(K_radius,
                                                     weakly_num_kpoints,
                                                     num_kernels=1,
                                                     dimension=3,
                                                     fixed='center')

        K_points_numpy = weakly_K_points_numpy.reshape((weakly_num_kpoints, 3))
        ...
        neighbors = neighbor_xyz - tf.expand_dims(xyz, 2)
        ...
        all_weights = tf.maximum(1 - tf.sqrt(sq_distances) / weakly_KP_extent, 0.0)
        ...
        kernel_outputs = tf.matmul(weighted_features, weakly_w)
        ...
        rs_map_s1 = tf.norm(output_features, ord=2, axis=-1, keepdims=True)
        rs_mapf1 = output_features / rs_map_s1
        rs_mapf2 = rs_mapf1
        fea_out = tf.concat([feature_sq, output_features], axis=-1)

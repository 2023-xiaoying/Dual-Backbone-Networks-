...
class Network:
    def __init__(self, dataset, config):
        ...
        if self.config.saving:
            if self.config.saving_path is None:
                self.saving_path = time.strftime('results/PSD_Log_%Y-%m-%d', time.gmtime())
        ...
        with tf.variable_scope('layers'):
            self.logits, self.embedding, self.embedding2, self.logits_e = self.inference(self.inputs, self.is_training)
        ...
        with tf.variable_scope('loss'):
            ...
            self.logits1_e = tf.reshape(self.logits_e, [-1, config.num_classes])
            ...
            valid_logits_e = tf.gather(self.logits1_e, valid_idx, axis=0)
            ...
            self.loss = self.get_loss(valid_logits, valid_logits_e, valid_labels, embedding, embedding2, global_labels,
                                      self.logits_out, self.logits_e,
                                      self.class_weights)
    ...
    def inference(self, inputs, is_training):
        ...
        feature_e = inputs['features']

        feature_e = tf.layers.dense(feature_e, 8, activation=None, name='fc0_e')
        feature_e = tf.nn.leaky_relu(tf.layers.batch_normalization(feature_e, -1, 0.99, 1e-6, training=is_training))
        feature_e = tf.expand_dims(feature_e, axis=2)

        # ###########################Encoder############################
        f_encoder_list = []
        for i in range(self.config.num_layers):
            xyz, neigh_idx, sub_idx = self.input_element(is_training, inputs['xyz'][i], inputs['neigh_idx'][i],
                                                         inputs['sub_idx'][i])
        ...
        f_encoder_list_e = []
        for i_e in range(3):
            xyz_e, neigh_idx_e, sub_idx_e = inputs['xyz'][i_e], inputs['neigh_idx'][i_e], inputs['sub_idx'][i_e]
            f_encoder_i_e = self.dilated_res_block(feature_e, xyz_e, neigh_idx_e, d_out_e[i_e],
                                                   'Encoder_layer_e_' + str(i_e), is_training)
            f_sampled_i_e = self.random_sample(f_encoder_i_e, sub_idx_e)
            feature_e = f_sampled_i_e
            if i_e == 0:
                f_encoder_list_e.append(f_encoder_i_e)
            f_encoder_list_e.append(f_sampled_i_e)

        feature = helper_tf_util.conv2d(f_encoder_list[-1], f_encoder_list[-1].get_shape()[3].value, [1, 1],
                                        'decoder_0',
                                        [1, 1], 'VALID', True, is_training)

        feature_e = helper_tf_util.conv2d(f_encoder_list_e[-1], f_encoder_list_e[-1].get_shape()[3].value, [1, 1],
                                          'decoder_0_e',
                                          [1, 1], 'VALID', True, is_training)
        ...
        for j in range(self.config.num_layers):
            # interp_idx = inputs['interp_idx'][-j - 1]
            interp_idx = tf.cond(is_training,
                                 lambda: tf.concat([inputs['interp_idx'][-j - 1], inputs['interp_idx'][-j - 1]],
                                                   axis=0),
        ...
        f_decoder_list_e = []
        for j_e in range(3):
            interp_idx_e = inputs['interp_idx'][-j_e - 3]
            f_interp_i_e = self.nearest_interpolation(feature_e, interp_idx_e)
            f_decoder_i_e = helper_tf_util.conv2d_transpose(
                tf.concat([f_encoder_list_e[-j_e - 2], f_interp_i_e], axis=3),
                f_encoder_list_e[-j_e - 2].get_shape()[-1].value, [1, 1],
                'Decoder_layer_e_' + str(j_e), [1, 1], 'VALID', bn=True,
                is_training=is_training)
            feature_e = f_decoder_i_e
            f_decoder_list_e.append(f_decoder_i_e)
        ...
        f_layer_fc4 = helper_tf_util.conv2d(f_layer_drop, self.config.num_classes, [1, 1], 'fc_0', [1, 1], 'VALID',
                                            False,
                                            is_training, activation_fn=None)
        f_out = tf.squeeze(f_layer_fc4, [2])

        f_layer_fc1_e = helper_tf_util.conv2d(f_decoder_list_e[-1], 32, [1, 1], 'fc1_e', [1, 1], 'VALID', True,
                                              is_training)
        # f_layer_fc1_e, rs_mapf1, rs_mapf2 = self.srcontext(f_layer_fc1, inputs['neigh_idx'][0], is_training)
        f_layer_fc2_e = helper_tf_util.conv2d(f_layer_fc1_e, 32, [1, 1], 'fc2_e', [1, 1], 'VALID', True, is_training)

        f_layer_drop_e = helper_tf_util.dropout(f_layer_fc2_e, keep_prob=0.5, is_training=is_training, scope='dp1_e')
        f_layer_fc4_e = helper_tf_util.conv2d(f_layer_drop_e, self.config.num_classes, [1, 1], 'fc_0_e', [1, 1],
                                              'VALID',
                                              False,
                                              is_training, activation_fn=None)
        f_out_e = tf.squeeze(f_layer_fc4_e, [2])

        return f_out, rs_mapf1, rs_mapf2, f_out_e
    ...
    def get_embedding(self, point_cloud, d_out, neigh_idx, name, is_training, activation_fn=None):
        ...
        net = helper_tf_util.conv2d(edge_feature, d_out, [1, 1], name, [1, 1], 'VALID', False, is_training,
                                    activation_fn)
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                a = 1 / 0
        t = open(
            dataset.experiment_dir + '/best_miou_{:5.3f}_epoch_{}.txt'.format(max(self.mIou_list), self.best_epoch),
            'a')
        t.close()
        print('finished')
        self.sess.close()
    ...
    def get_loss(self, logits, vail_logits_e, valid_labels, embedding1, embedding2, global_labels, logits_out, logits_e,
                 pre_cal_weights):
        ...
        # 第二个backebone的CE loss
        labels = valid_labels
        class_weights = tf.convert_to_tensor(pre_cal_weights, dtype=tf.float32)
        one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)
        weights = tf.reduce_sum(class_weights * one_hot_labels, axis=1)
        unweighted_losses_e = tf.nn.softmax_cross_entropy_with_logits(logits=vail_logits_e, labels=one_hot_labels)
        weighted_losses_e = unweighted_losses_e * weights
        CE_loss_e = tf.reduce_mean(weighted_losses_e)
        ...
        # 双backbone的预测计算损失
        logits_e = tf.nn.softmax(logits_e, axis=-1)
        p2_e = logits_e
        q_e = 1 / 2 * (p1 + p2_e)
        loss_kl_e = tf.reduce_mean(p1 * tf.log(p1 / (q_e + 1e-4) + 1e-4) + p2_e * tf.log(p2_e / (q_e + 1e-4) + 1e-4))

        loss_cr = Network.cr_loss(embedding1, embedding2, global_labels, self.config.num_classes)
        tf.summary.scalar('loss_cr', loss_cr)
        output_loss = CE_loss + CE_loss_e + loss_kl + loss_cr + loss_kl_e + lovas_loss
        return output_loss
    ...
    def data_augment(self, data):
        ...
        data_xyz = data[:, :, 0:3]
        data_f = data[:, :, 3:]
        batch_size = data_f.get_shape()[0].value
        ...
        if mirror_opt == 0:
            ...
            data_xyz = self.region_wise_masking(data_xyz)
    ...
    def region_wise_masking(self, data_xyz):
        """
        对点云数据进行 region-wise masking。
        :param data_xyz: 点云的坐标数据，形状为 [batch_size, num_points, 3]
        :return: 屏蔽后的点云坐标数据
        """
        batch_size = data_xyz.shape[0]
        num_points = data_xyz.shape[1]

        # 假设我们将点云空间划分为8x8x8的网格
        grid_size = 8  # 可以调节网格的大小
        full_scale = 1.0  # 设定点云的尺度范围（例如 [0, 1]）

        # 随机选择要屏蔽的区域
        mask_ratio = 0.2  # 屏蔽比例
        num_masked_points = int(num_points * mask_ratio)

        # 随机选择要屏蔽的区域索引
        mask_indices = np.random.choice(range(num_points), num_masked_points, replace=False)
        ...
        for i in range(batch_size):
            ...
            # 随机选择一个区域并进行屏蔽
            masked_coords = np.copy(coords)
            masked_coords[mask_indices] = 0  # 将被屏蔽的点的坐标置为0或其他值

            # 更新当前batch的坐标
            data_xyz[i, :, :] = masked_coords
